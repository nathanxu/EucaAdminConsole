...
public class CredentialAction {
	...
	public void doDownload(Context context, HttpServletResponse response, 
		...
		try {
			...
			if(!message.getStatus() || message.getErrMessage()!=null) {
				response.reset();
				response.setContentType("Content-Type: text/html;charset=UTF-8");
				response.setHeader("Cache-Control", "no_cache");
				OutputStream output = response.getOutputStream();			
				output.write("<html><body>\n".getBytes());
				//String script = "$(window.parent.document).notifyError(\""+message.getErrMessage()+"\");"; 
				String script = "alert(\"" + message.getErrMessage() +"\");";
				output.write(("<script type=\"text/javascript\" language=\"javascript\">"+script+"</script>").getBytes());
	...
	public void doModifyPassword(Context context,
		...
		try {
			...
			result.put("state", message.getStatus());
