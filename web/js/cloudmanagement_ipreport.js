...
(function($, eucalyptus) {
  $.widget('eucalyptus.cloudmanagement_ipreport', $.eucalyptus.eucawidget, {
    options : { },
    baseTable : null,
    tableWrapper : null,
    _init : function() {
      thisObj = this;
      var $tmpl = $('html body div.templates').find('#ipReportTmpl').clone();    
      var $wrapper = $($tmpl.render($.i18n.map));
      var $report = $wrapper.children().first();
      var $help = $wrapper.children().last(); 
      
      require(['app'], function(app) { 
          var report = app.data.cloudmanagement_ipreport;
          report.url = 'ea.cloudmanagement.IpReportAction$query.json';
          report.fetch();
          });      
      
      var $instTable = $report.children('.inner-table');
      thisObj.tableWrapper = $instTable.innertable({
          id : 'cloudmanagement_ipreport', // user of this widget should customize these options,
          data_deps: ['cloudmanagement_ipreport'],
          hidden: thisObj.options['hidden'],
          dt_arg : {
            "sAjaxSource": 'cloudmanagement_ipreport',
            "aoColumnDefs": [
              {
                "aTargets":[0],
      ...
      var $searchArea = $report.find('.table_cloudmanagement_ipreport_top');
      ...
      $searchArea.find('#seachRange').click(function() {
          require(['app'], function(app) { 
              var report = app.data.cloudmanagement_ipreport;
              report.url = 'ea.cloudmanagement.IpReportAction$query.json?from='+ $from.val() +'&to=' + $to.val();
              report.fetch();
