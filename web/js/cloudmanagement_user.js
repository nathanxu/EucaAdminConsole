...
(function($, eucalyptus) {
  $.widget('eucalyptus.cloudmanagement_user', $.eucalyptus.eucawidget, {
    options : { },
    baseTable : null,
    tableWrapper : null,
    addDialog : null,
    ...
    _init : function() {
      var thisObj = this;
      var $tmpl = $('html body div.templates').find('#userTmpl').clone();    
      ...
      thisObj.tableWrapper = $instTable.innertable({
          id : 'cloudmanagement_user', // user of this widget should customize these options,
          data_deps: ['cloudmanagement_user'],
          hidden: thisObj.options['hidden'],
          dt_arg : {
            "sAjaxSource": 'cloudmanagement_user',
            "aaSorting": [[ 3, "desc" ]],
            "aoColumnDefs": [
              {
                "bSortable": false,
              ...
              },
              {
                  ...
                  "mData": function(source){
                    var groupStr = '';
                    if (source.userGroups.length > 0)
                    	groupStr = source.userGroups[0].groupName;
                    $.each(source.userGroups, function(idx, val) {
                        if (idx > 0 )
                        	groupStr += ("," + val.groupName);
                    });
                    return groupStr;
                  },
                },
          ...
          menu_click_create : function(e) {
        	  ...
        	  var groups = describe('cloudmanagement_usergroup');
        	  var $accountArea = thisObj.addDialog.find('#account');
        	  $accountArea.empty();
        	  $.each(accounts, function(idx, val) {
        		  $accountArea.append($('<option>').val(val.accountName).text(val.accountName));
        	  });
				...
				return {
					'Delete' : {
						...
						callback : function(key, opt) {
      				        ...
      				        if ( itemsToDelete.length > 0 ) {
      				          thisObj.delDialog.eucadialog('setSelectedResources', {title:[user_subtitle], contents: matrix, limit:60, hideColumn: 1});
      				          thisObj.delDialog.dialog('open');
      				        }
						}
					},
					'Manage_UserGroup' : {
						"name" : user_manageusergroup,
						callback : function(key, opt) {
							var items = thisObj.tableWrapper.innertable('getSelectedRows');
							//alert(items[0].userAccount);
							if (items.length != 1) {
								notifySuccess($.i18n.prop('error_msg_select_one', $.i18n.map.user_subtitle));
								return false;
							} 
							var matrix = [];
							var accountGroups = [];
  				            $.ajax({
  				            type:"POST",
  				            url:"ea.cloudmanagement.UserAction$queryUserGroup.json",
  				            data:{account: items[0].userAccount, user: items[0].userName},
  				            dataType:"json",
  				            async:false,
      				          success:function(data){
                                  ...
                                  if (groups){
                                    for(i in groups){
                                      if(groups[i].groupAccount.toString() == items[0].userAccount) {	
                                    	  accountGroups.push(groups[i]);
                                      }
                                    }  
                                  }
      				        	  $.each(accountGroups, function(idx1,ug) {
      				        	      var flg = false;
          				        	  $.each(data, function(idx2, checkedUg) {
          				        		  if (checkedUg.groupName == ug.groupName) {
          				        		      flg = true;
          				        		      return false;
          				        		  }
            				          });
            				          if (flg) {
			                             matrix.push([ug.groupName,
                                             $.i18n.map.usergroup_name+': '+ug.groupName+'\n'+
                                             $.i18n.map.usergroup_account+': '+ug.groupAccount,
                                             true]);   
            				          } else {
                                         matrix.push([ug.groupName,
                                             $.i18n.map.usergroup_name+': '+ug.groupName+'\n'+
  				            ...
  				            thisObj.addUserGroupDialog.eucadialog('setSelectedResourcesWithCheckbox', {contents: matrix});
      				        if ( matrix.length > 0 ) {
      				          //thisObj.addUserGroupDialog.eucadialog('setSelectedResourcesWithCheckbox', {contents: matrix});
      				          thisObj.addUserGroupDialog.find(".selected-resources").hcheckbox();
      				        }
							thisObj.addUserGroupDialog.dialog('open');
						}
					},
					'Add_Policy' : {
						"name" : user_addpolicy,
						callback : function(key, opt) {
                                ...
                                thisObj.addPolicyDialog.find('#account').val(items[0].userAccount);
                                thisObj.addPolicyDialog.find('#user').val(items[0].userName);
					...
					'Delete_Policy' : {
                        ...
                        callback : function(key, opt) {
                                ...
                                $.ajax({
                                    type:"POST",
                                    url:"ea.cloudmanagement.PolicyAction$getPolicysByUser.json",
                                    data:{user: items[0].userName, account: items[0].userAccount},
                                    dataType:"json",
                                    async:false,
                                    success:function(data){
    ...
    _create : function() { 
        ...
        thisObj.delDialog = $del_dialog.eucadialog({
           id: 'keys-delete',
        ...
        thisObj.addDialog = $add_dialog.eucadialog({
            id: 'keys-add',
        ...
        thisObj.addUserGroupDialog = $addUserGroup_dialog.eucadialog({
           id: 'keys-delete',
           width: 500,
           title: user_manageusergroup,
           buttons: {
             'save': {
            	 text: button_save, 
            	 click: function() {
            	  var items = thisObj.tableWrapper.innertable('getSelectedRows');
                  var group = thisObj.addUserGroupDialog.eucadialog('getSelectedCheckedResources');
                  if (items.length < 1) {
                	  thisObj.addUserGroupDialog.eucadialog('showError',error_msg_multiple_delete);
                  } else {
                	  $addUserGroup_dialog.eucadialog("close");
                	  var model = {account: items[0].userAccount, user: items[0].userName, group: group};
                      thisObj._userGroupAction(model);
                  }
               }},
             'cancel': {text: button_cancel, focus:true, click: function() { $addUserGroup_dialog.eucadialog("close");}} 
           },
         });
         
        var createPolicyButtonId = "keys-add-policy-btn";
        var $tmpl = $('html body').find('#addUserPolicyDlgTmpl').clone();
        var $rendered = $($tmpl.render($.extend($.i18n.map)));
             ...
             'delete': {text: button_delete, click: function() {
                  ...
                  if (policys == '') {
                      ...
                      $.each(policys.split(','), function(idx, val) {
                        var model = {account: items[0].userAccount, user: items[0].userName, policy: val};
    ...
    _addAction : function(model) {
     ...
     _userGroupAction: function(model) {
         $.ajax({
             type:"POST",
             url:"ea.cloudmanagement.UserAction$manageUserGroup.json",
             data:model,
             dataType:"json",
             async:false,
             success:
