...
(function($, eucalyptus) {
  $.widget('eucalyptus.maincontainer', {
    ...
    changeInnerSelected : function (evt, ui) {
    	var container = ui.container;
    	var innerselected = ui.selected;
    	container.children().detach();
    	eval("container."+innerselected+"()");
    },
    updateSelected : function (selected, filter, options) {
     ...
     switch(selected){
         case 'dashboard':
             this.element.dashboard({select: function(evt, ui){$container.maincontainer("changeSelected", evt, ui)},innerselect: function(evt, ui){$container.maincontainer("changeInnerSelected", evt, ui)}});
             break;
         case 'infrastructure':
             this.element.infrastructure({innerselect: function(evt, ui){$container.maincontainer("changeInnerSelected", evt, ui)}});
             break;
         case 'configurecloud':
        	 this.element.configurecloud({innerselect: function(evt, ui){$container.maincontainer("changeInnerSelected", evt, ui)}});
        	 break;
         case 'cloudmanagement':
        	 this.element.cloudmanagement({innerselect: function(evt, ui){$container.maincontainer("changeInnerSelected", evt, ui)}});
        	 break;
         case 'instance':
