...
(function($, eucalyptus) {
  $.widget('eucalyptus.cloudmanagement_volumereport', $.eucalyptus.eucawidget, {
    options : { },
    baseTable : null,
    tableWrapper : null,
    _init : function() {
      thisObj = this;
      var $tmpl = $('html body div.templates').find('#volumeReportTmpl').clone();    
      var $wrapper = $($tmpl.render($.i18n.map));
      var $report = $wrapper.children().first();
      var $help = $wrapper.children().last(); 
      
      require(['app'], function(app) { 
          var report = app.data.cloudmanagement_volumereport;
          report.url = 'ea.cloudmanagement.VolumeReportAction$query.json';
          report.fetch();
          });      
      
      var $instTable = $report.children('.inner-table');
      thisObj.tableWrapper = $instTable.innertable({
          id : 'cloudmanagement_volumereport', // user of this widget should customize these options,
          data_deps: ['cloudmanagement_volumereport'],
          hidden: thisObj.options['hidden'],
          dt_arg : {
            "sAjaxSource": 'cloudmanagement_volumereport',
            "aoColumnDefs": [
              {
                "aTargets":[0],
      ...
      var $searchArea = $report.find('.table_cloudmanagement_volumereport_top');
      ...
      $searchArea.find('#seachRange').click(function() {
          require(['app'], function(app) { 
              var report = app.data.cloudmanagement_volumereport;
              report.url = 'ea.cloudmanagement.VolumeReportAction$query.json?from='+ $from.val() +'&to=' + $to.val();
              report.fetch();
