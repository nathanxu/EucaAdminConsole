...
(function($, eucalyptus) {
  $.widget('eucalyptus.infrastructure_vmwarebrokers', $.eucalyptus.eucawidget, {
    options : { },
    tableWrapper : null,
    addDialog : null,
    _init : function() {
      thisObj = this;
      var $tmpl = $('html body div.templates').find('#vmwareBrokersTmpl').clone();       
      ...
      thisObj.tableWrapper = $instTable.innertable({
          id : 'vmwareBrokers', // user of this widget should customize these options,
          data_deps: [],
          hidden: thisObj.options['hidden'],
          dt_arg : {
            "sAjaxSource": 'infrastructure_vmwarebroker',
            "aaSorting": [[ 1, "desc" ]],
            "aoColumnDefs": [
              {
 				...
 				'Delete' : {
  					...
  					callback : function() {
  				        ...
  				        if ( itemsToDelete.length > 0 ) {
  				          thisObj.delDialog.eucadialog('setSelectedResources', {title:[title_vmbroker], contents: matrix, limit:60, hideColumn: 1});
    ...
    _create : function() { 
        ...
        var $tmpl = $('html body div.templates').find('#addVMBDlgTmpl').clone();       
        $tmpl.find('#host-user, #password').remove();
        var $rendered = $($tmpl.render($.extend($.i18n.map)));
        var $add_dialog = $rendered.children().first();
        this.addDialog = $add_dialog.eucadialog({
            id: 'keys-add',
            title: title_add_vmbroker,
            width: 500,
            buttons: { 
		    ...
		    'create' : {
								...
								click : function() {
									...
									var model = {host: host, name: name, partition:partition};
        ...
        $add_dialog.eucadialog('buttonOnKeyupNew', $add_dialog.find('#ip'), createButtonId, function(val){
            ...
            var isMatch = IP_PATTERN.test(ip) && INFRASTRACTURE_NAME_PATTERN.test(name)
            	&& INFRASTRACTURE_NAME_PATTERN.test(partition) && INFRASTRACTURE_NAME_PATTERN.test(bm);
        ...
        $add_dialog.eucadialog('buttonOnKeyupNew', $add_dialog.find('#name'), createButtonId, function(val){
          ...
          var partition = $.trim($add_dialog.find('#partition').val());
          var isMatch = IP_PATTERN.test(ip) && INFRASTRACTURE_NAME_PATTERN.test(name)
          	&& INFRASTRACTURE_NAME_PATTERN.test(partition);
        ...
        $add_dialog.eucadialog('buttonOnKeyupNew', $add_dialog.find('#block-manager'), createButtonId, function(val){
            var name = $.trim($add_dialog.find('#name').val());
            var ip = $.trim($add_dialog.find('#ip').val());
            var partition = $.trim($add_dialog.find('#partition').val());
            var isMatch = IP_PATTERN.test(ip) && INFRASTRACTURE_NAME_PATTERN.test(name)
    ...
    _disableAction : function() {
        ...
        $.ajax({
        	type:"POST",
        	url:"ea.cluster.ClusterAction$disableVmwarebroker.json",
            ...
            function(data, textStatus, jqXHR){
              ...
              require(['app'], function(app) { 
              	app.data.infrastructure_vmwarebroker.fetch(); 
              });
    ...
    _enableAction : function() {
        ...
        $.ajax({
        	type:"POST",
        	url:"ea.cluster.ClusterAction$enableVmwarebroker.json",
            data:{items : itemsToDelete},
            dataType:"json",
            async:false,
            success:
    ...
    _deleteAction : function(json) {
        $.ajax({
        	type:"POST",
        	url:"ea.cluster.ClusterAction$deleteVmwarebroker.json",
            data:{json : json},
            dataType:"json",
            async:false,
            success:
    ...
    _addAction : function(model) {
        $.ajax({
          type:"POST",
          url:"ea.cluster.ClusterAction$addVmwarebroker.json",
          data:model,
          dataType:"json",
          async:false,
          success:
