...
(function($, eucalyptus) {
  $.widget('eucalyptus.infrastructure_clustercontrollers', $.eucalyptus.eucawidget, {
    options : { },
    tableWrapper : null,
    addDialog : null,
    delDialog : null,
    _init : function() {
      thisObj = this;
      var $tmpl = $('html body div.templates').find('#clustercontrollersTmpl').clone();       
      ...
      thisObj.tableWrapper = $instTable.innertable({
          id : 'clustercontrollers', // user of this widget should customize these options,
          data_deps: [],
          hidden: thisObj.options['hidden'],
          dt_arg : {
            "sAjaxSource": 'infrastructure_cc',
            "aaSorting": [[ 1, "desc" ]],
            "aoColumnDefs": [
              {
      				...
      				'Delete' : {
      					...
      					callback : function() {
      				        ...
      				        if ( itemsToDelete.length > 0 ) {
      				          thisObj.delDialog.eucadialog('setSelectedResources', {title:[title_cc], contents: matrix, limit:60, hideColumn: 1});
    ...
    _create : function() { 
        ...
        var $tmpl = $('html body div.templates').find('#addCCDlgTmpl').clone();       
        var $rendered = $($tmpl.render($.extend($.i18n.map)));
        ...
        this.addDialog = $add_dialog.eucadialog({
		    ...
		    'create' : {
								...
								click : function() {
									...
									var model = {host:host, name: name, partition:partition, hostUser:hostUser, password:password};
    ...
    _disableAction : function() {
        ...
        $.ajax({
        	type:"POST",
        	url:"ea.cluster.ClusterAction$disableClustercontroller.json",
            ...
            function(data, textStatus, jqXHR){
              ...
              require(['app'], function(app) { 
              	app.data.infrastructure_cc.fetch(); 
              });
    ...
    _enableAction : function() {
        ...
        $.ajax({
        	type:"POST",
        	url:"ea.cluster.ClusterAction$enableClustercontroller.json",
            data:{items : itemsToDelete},
            dataType:"json",
            async:false,
            success:
    ...
    _deleteAction : function(json) {
        $.ajax({
        	type:"POST",
        	url:"ea.cluster.ClusterAction$deleteClustercontroller.json",
            data:{json : json},
            dataType:"json",
            async:false,
            success:
    ...
    _addAction : function(model) {
        $.ajax({
          type:"POST",
          url:"ea.cluster.ClusterAction$addClustercontroller.json",
          data:model,
          dataType:"json",
          async:false,
          success:
