...
(function($, eucalyptus) {
  $.widget('eucalyptus.cloudmanagement_instancereport', $.eucalyptus.eucawidget, {
    options : { },
    baseTable : null,
    tableWrapper : null,
    _init : function() {
      thisObj = this;
      var $tmpl = $('html body div.templates').find('#instanceReportTmpl').clone();    
      var $wrapper = $($tmpl.render($.i18n.map));
      var $report = $wrapper.children().first();
      var $help = $wrapper.children().last(); 
      
      require(['app'], function(app) { 
          var report = app.data.cloudmanagement_instancereport;
          report.url = 'ea.cloudmanagement.InstanceReportAction$query.json';
          report.fetch();
      });
      
      var $instTable = $report.children('.inner-table');
      thisObj.tableWrapper = $instTable.innertable({
          id : 'cloudmanagement_instancereport', // user of this widget should customize these options,
          data_deps: ['cloudmanagement_instancereport'],
          hidden: thisObj.options['hidden'],
          dt_arg : {
            "sAjaxSource": 'cloudmanagement_instancereport',
            "aoColumnDefs": [
              {
            	"aTargets":[0],
                "mData": function(source){
                  return eucatableDisplayColumnTypeTwist($.i18n.map.title_detail, source.instanceId, 256); 
                },
              },
              {
                "aTargets":[1],
      ...
      var $searchArea = $report.find('.table_cloudmanagement_instancereport_top');
      ...
      $searchArea.find('#seachRange').click(function() {
          if ($from.val() != '' && $to.val() != '') {
              require(['app'], function(app) { 
                  var report = app.data.cloudmanagement_instancereport;
                  report.url = 'ea.cloudmanagement.InstanceReportAction$query.json?from='+ $from.val() +'&to=' + $to.val();
                  report.fetch();
      ...
      _expandCallback : function(row){ 
        var $el = $('<div />');
        var result= $('html body').eucadata('get', 'cloudmanagement_instancereport');
        var model = null;
        if (result){
          for(i in result){
            var id = result[i].instanceId.toString();
            if(id && id.toUpperCase() === row[4].toString().toUpperCase())
              model = result[i];
          }
        }
        require(['app', 'views/expandos/cloudmanagement_instancereport'], function(app, expando) {
            new expando({
                el : $el,
                model : model
            });
