...
//    if(!options.script || !options.keyname || !options._xsrf){
    if(!options.script || !options.keyname){
    ...
    // down credential
    $.downCredential = function(options){
    options = options || {};

    if(!options.script || !options.userName || !options.accountName){
      throw new Error("Please enter all the required config options!");
    }
    ...
    // The body element of the iframe document:
      ...
      form.attr('action',options.script);
      form.find('input[name=userName]').val(options.userName);
      form.find('input[name=accountName]').val(options.accountName);
