...
(function($, eucalyptus) {
  $.widget('eucalyptus.infrastructure_storagecontrollers', $.eucalyptus.eucawidget, {
    options : { },
    tableWrapper : null,
    addDialog : null,
    isFirstSC : true,
    _init : function() {
      thisObj = this;
      var $tmpl = $('html body div.templates').find('#storagecontrollersTmpl').clone();       
      ...
      thisObj.tableWrapper = $instTable.innertable({
          id : 'storagecontrollers', // user of this widget should customize these options,
          data_deps: [],
          hidden: thisObj.options['hidden'],
          dt_arg : {
            "sAjaxSource": 'infrastructure_sc',
//            "aaSorting": [[ 1, "asc" ]],
            "aoColumnDefs": [
              {
    ...
    _create : function() { 
        ...
        var $tmpl = $('html body div.templates').find('#addSCDlgTmpl').clone();       
        var $rendered = $($tmpl.render($.extend($.i18n.map)));
...
//            $.ajax({
//                type:"POST",
//                url:"ea.cluster.StorageAction$queryBlockManagerByPartition.json",
//                data:{partition : partition},
//                dataType:"json",
//                async:false,
//                success: function(data){
//                	if (data != null) {
//                		$add_dialog.find('#block-manager').parent().hide();
//                		$add_dialog.find('#block-manager').val(data.mode);
//                		thisObj.isFirstSC = false;
//                	} else {
//                		$add_dialog.find('#block-manager').parent().show();
//                		thisObj.isFirstSC = true;
//                	}
//                },
//             });
            return isMatch;
    ...
    _disableAction : function() {
        ...
        $.ajax({
        	type:"POST",
        	url:"ea.cluster.ClusterAction$disableStoragecontroller.json",
            ...
            function(data, textStatus, jqXHR){
              ...
              require(['app'], function(app) { 
              	app.data.infrastructure_sc.fetch(); 
              });
    ...
    _enableAction : function() {
        ...
        $.ajax({
        	type:"POST",
        	url:"ea.cluster.ClusterAction$enableStoragecontroller.json",
            data:{items : itemsToDelete},
            dataType:"json",
            async:false,
            success:
    ...
    _deleteAction : function(json) {
        $.ajax({
        	type:"POST",
        	url:"ea.cluster.ClusterAction$deleteStoragecontroller.json",
            data:{json : json},
            dataType:"json",
            async:false,
            success:
    ...
    _addAction : function(model) {
        $.ajax({
          type:"POST",
          url:"ea.cluster.ClusterAction$addStoragecontroller.json?isFirstSC=" + thisObj.isFirstSC,
          data:model,
          dataType:"json",
          async:false,
          success:
