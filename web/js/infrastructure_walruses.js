...
(function($, eucalyptus) {
  $.widget('eucalyptus.infrastructure_walruses', $.eucalyptus.eucawidget, {
    options : { },
    tableWrapper : null,
    _init : function() {
      thisObj = this;
      var $tmpl = $('html body div.templates').find('#infrastructureWalrusesTmpl').clone();       
      ...
      thisObj.tableWrapper = $instTable.innertable({
          id : 'infrastructureWalruses', // user of this widget should customize these options,
          data_deps: [],
          hidden: thisObj.options['hidden'],
          dt_arg : {
            "sAjaxSource": 'infrastructure_walrus',
            "aaSorting": [[ 1, "asc" ]],
            "aoColumnDefs": [
              {
      				...
      				'Delete' : {
      					...
      					callback : function() {
      				        ...
      				        if ( itemsToDelete.length > 0 ) {
      				          thisObj.delDialog.eucadialog('setSelectedResources', {title:[title_walrus], contents: matrix, limit:60, hideColumn: 1});
        ...
        this.addDialog = $add_dialog.eucadialog({
		    ...
		    'create' : {
								...
								click : function() {
									...
									var model = {host:host, name: name, hostUser:hostUser, password:password};
    ...
    _disableAction : function() {
        ...
        $.ajax({
        	type:"POST",
        	url:"ea.cloud.CloudComponentsAction$disableWalrus.json",
            ...
            function(data, textStatus, jqXHR){
              ...
              require(['app'], function(app) { 
              	app.data.infrastructure_walrus.fetch(); 
              });
    ...
    _enableAction : function() {
        ...
        $.ajax({
        	type:"POST",
        	url:"ea.cloud.CloudComponentsAction$enableWalrus.json",
            data:{items : itemsToDelete},
            dataType:"json",
            async:false,
            success:
    ...
    _deleteAction : function(itemsToDelete) {
        $.ajax({
        	type:"POST",
        	url:"ea.cloud.CloudComponentsAction$deleteWalrus.json",
            data:{items : itemsToDelete},
            dataType:"json",
            async:false,
            success:
    ...
    _addAction : function(model) {
        $.ajax({
          type:"POST",
          url:"ea.cloud.CloudComponentsAction$addWalrus.json",
          data:model,
          dataType:"json",
          async:false,
          success:
