...
(function($, eucalyptus) {
  $.widget('eucalyptus.innertable', {
    ...
    _init : function() {
      ...
      if(thisObj.options.edit_args){
    	  this.table.makeEditable(thisObj.options.edit_args);    	  
      }
...
//      tableRefreshCallback = thisObj.refreshCallback;
    ...
    _decorateSearchBar : function(args) {//装饰搜索栏
    ...
    _decorateTopBar : function(args) {//装饰顶部操作栏--包括按钮们
      var thisObj = this; // ref to widget instance
  ...
  */
  _decorateActionMenu : function (args){
    ...
    _decorateLegendPagination : function (args) {//装饰底部提示条
      var thisObj = this;
...
/**** Public Methods ****/
    ...
    getSelectedRowsString : function (columnIdx) {
        var dataTable = this.table;
        if ( !dataTable )
          return '';
        var rows = dataTable.fnGetVisibleTrNodes();
        ...
        for ( i = 0; i<rows.length; i++ ) {
          ...
          if ( cb != null && cb.checked == true ) {
              if (selectedRows == '') {
            	  selectedRows =  dataTable.fnGetData(rows[i], columnIdx);
              } else {
            	  selectedRows += (',' + dataTable.fnGetData(rows[i], columnIdx));
              }
          }
