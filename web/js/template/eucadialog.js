...
/**** Public Methods ****/
    ...
    /// 
    /// resources ={title:[ , ], contents:[[val0_0, val0_1, .. ], [val1_0, val1_2, ..]], hideColumn: 1, limit:50
    setSelectedResourcesWithCheckbox : function (resources) {
      var thisObj = this;
      var $div = this.element.find('.selected-resources');
      if (! $div){ return false; }
      $div.children().detach();
      var $table = $('<table>');
      $div.append($table);
      var maxLimit = resources.limit ? resources.limit : 10;
      var $tr = $('<tr>');
      $.each(resources.contents, function(i, row){//row[0]:name row[1]:title row[2] checked
	    if (i % 4 == 0){
	        $tr = $('<tr>');
	    }
	    var $td = $('<td>');
	    $td.append($('<input type="checkbox">').attr('checked', row[2])).append($('<label>').attr('id', row[0]).attr('title', row[1]).text(addEllipsis(row[0],maxLimit)));
        $tr.append($td);
        $table.append($tr);
      });
      $div.append($table);
    ...
    getSelectedCheckedResources : function () {
        var thisObj = this;
        var $body = thisObj.element.find('tbody');
        var string = '';
        $body.find('td .checkbox.checked').each( function() {
    ...
    getSelectedResources : function (column) {
      ...
      if (typeof(column) == "undefined") {
          $body.find('tr').each( function() {
            array.push([$(this).find('td:eq(0)').text(), $(this).find('td:eq(1)').text()]);
          });           
      } else {
          $body.find('tr').each( function() {
    ...
    getSelectedResourcesString : function (column) {
        var thisObj = this;
        var $body = thisObj.element.find('tbody');
        var string = '';
        $body.find('tr').each( function() {
